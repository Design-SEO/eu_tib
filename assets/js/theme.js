...
// Theme
window.theme = {};
window.theme.$ = window.theme.$ || window.jQuery || window.$;
...
window.theme.fn = {
  ...
  intObs: function (selector, functionName, intObsOptions, alwaysObserve) {
    var themeJQ = window.theme.$ || window.jQuery || window.$;
    ...
    if (Object.keys(intObsOptions).length) {
      intersectionObserverOptions = themeJQ.extend(
      ...
      for (var i = 0; i < entries.length; i++) {
        ...
        if (entry.intersectionRatio > 0) {
          if (typeof functionName === "string") {
            ...
            callback.call(themeJQ(entry.target));
          }
    ...
    themeJQ($el).each(function () {
      observer.observe(themeJQ(this)[0]);
    });
  },
  ...
  execOnceTroughEvent: function ($el, event, callback) {
    ...
    themeJQ($el).on(event, function () {
      if (!themeJQ(this).data(dataName)) {
        // Exec Callback Function
        callback.call(themeJQ(this));

        // Add data name
        themeJQ(this).data(dataName, true);

        // Unbind event
        themeJQ(this).off(event);
      }
    });
  ...
  getScripts: function (arr, path) {
    var themeJQ = window.theme.$ || window.jQuery || window.$;
    var _arr = themeJQ.map(arr, function (scr) {
      return themeJQ.getScript((path || "") + scr);
    });

    _arr.push(
      themeJQ.Deferred(function (deferred) {
        themeJQ(deferred.resolve);
      })
    );

    return themeJQ.when.apply(themeJQ, _arr);
  },

  showErrorMessage: function (title, content) {
    var themeJQ = window.theme.$ || window.jQuery || window.$;
    themeJQ(".modalThemeErrorMessage").remove();
    themeJQ("body").append(
  ...
  if ($("[data-copy-to-clipboard]").length) {
    theme.fn.intObs(
      ...
      function () {
        ...
        var $copyButton = $(
          '<a href=\'<?php echo getHref("")\' ?>" class="btn btn-primary btn-px-2 py-1 text-0 position-absolute top-8 right-8">COPY</a>'
        );
